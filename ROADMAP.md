...
- [x] **Enable Cloudflare Access on `/admin`.** ✅ Done & verified — `/admin` redirects to the Cloudflare Access login; homepage, booking/services pages, and the signup/confirm/unsubscribe API stay public. Scoping is correct.
  ...
  - [x] **Confirmed:** Lumiderm Policy → Include → Emails (Action: Allow) contains `dima.vasiliu@yahoo.com` and `info@lumidermaesthetics.co.uk` (the clinic inbox = the clinic owner's login). the clinic owner signs in via a one-time code emailed to that inbox. Session duration 30 min; MFA off (email-OTP is the factor).
...
- [x] **Image uploads via Cloudflare R2 (media library).** ✅ Done. the clinic owner can upload any image in **Media** (real upload/list/delete), in the **Offers** editor (uploads now go to R2 and appear in the image picker), and as an optional **banner image** on email campaigns. Stored in R2 (not D1 — wrong tool for files), served publicly at `/media/<key>` with long cache headers so it works on the site *and* in email. Path-sanitised, 6MB/image-type limits, every upload/delete audit-logged. Offers pipeline handles `/media/…` URLs alongside stock assets. **Dima action: enable R2 + create the `lumiderm-media` bucket — see `MEDIA-SETUP.md`.** Verified: R2 key-gen/ext/path-traversal guards, offers.json image passthrough, all syntax.
...
- [x] **Birthday radar on the dashboard** ✅ Done. The Overview shows a **Birthdays** card listing confirmed + consented subscribers whose birthday is **today or within 7 days**, with a one-click **Send birthday email** button for today's (uses the saved birthday template, marks the year so the automation won't duplicate, audit-logged as `birthday.manual_send`). A toast + a "Needs attention" entry flag today's birthdays so she can't miss them. Works whether or not auto-send is on. Days-until/window logic verified against SQLite.
...
- [ ] **Confirm real offer content & prices** with the practitioner (some are still placeholder promos).
